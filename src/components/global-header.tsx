...
import { MOBILE_PRIMARY_LINKS, PRIMARY_LINKS, SITE_BRAND_NAME, SITE_HOTLINE_TEL, SITE_HOTLINE_TEXT, UTILITY_LINKS } from "~/lib/constants/site";
    ...
    <header className="sticky top-0 z-50 border-slate-200/80 border-b bg-white/90 backdrop-blur">
      <div className="border-slate-200/80 border-b bg-slate-50/95">
        <div className="mx-auto flex min-h-10 w-full max-w-7xl flex-col justify-center gap-2 px-4 py-2 text-slate-600 text-sm sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <nav aria-label="辅助导航" className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <Link className="font-medium text-slate-800 transition-colors hover:text-slate-950" href="/">
              {SITE_BRAND_NAME}
            </Link>
            {UTILITY_LINKS.map((link) => (
              <Link className="transition-colors hover:text-slate-950" href={link.href} key={link.label}>
                {link.label}
              </Link>
            ))}
          </nav>
          <Link className="text-left text-slate-500 text-xs transition-colors hover:text-slate-950 sm:text-sm lg:text-right" href={SITE_HOTLINE_TEL}>
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex min-h-20 items-center justify-between gap-4 py-3">
          <Link className="flex items-center gap-3 text-slate-950 transition-opacity hover:opacity-80" href="/">
            <Image alt="戴氏教育标志" className="h-20 w-auto object-contain" height={80} priority src="/logo.png" width={190} />
          </Link>
          ...
          <nav aria-label="主导航" className="hidden items-center gap-1 lg:flex">
            {PRIMARY_LINKS.map((link) =>
              "items" in link ? (
                ...
                <Link className="rounded-full px-4 py-2 font-medium text-slate-700 text-sm transition-colors hover:bg-slate-100 hover:text-slate-950" href={link.href} key={link.label}>
                  {link.label}
                </Link>
              ),
            )}
          ...
          <div className="hidden items-center sm:flex">
            <Button asChild className="bg-[#1d4ed8] hover:bg-[#1e40af]" size="lg">
              <Link href={SITE_HOTLINE_TEL}>立即咨询</Link>
            </Button>
          </div>
        </div>

        <nav aria-label="移动端主导航" className="flex gap-2 overflow-x-auto pb-3 lg:hidden">
