import { BarChart3Icon, BrainCircuitIcon, TargetIcon, ZapIcon } from "lucide-react";
          ...
          <div className="relative z-10 grid gap-16 lg:grid-cols-2 lg:items-center">
            <div>
              <div className="mb-6 flex items-center gap-2">
                ...
                <span className="font-bold text-primary text-sm uppercase tracking-wider">AI 智能诊断</span>
              </div>
              ...
              <p className="mb-12 max-w-xl text-lg text-slate-400 leading-relaxed">
                结合戴氏教育多年的教学经验，AI 帮孩子分析平时的做题数据，自动告诉家长和孩子：哪些知识点还没掌握、每天应该重点练什么、进步出现在哪里。
              </p>

              <div className="grid gap-6">
                {features.map((f) => (
                  <div className="flex gap-4" key={f.title}>
                    <div className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/5 text-primary">{f.icon}</div>
                    <div>
                      <h3 className="mb-1 font-bold text-lg text-white">{f.title}</h3>
                ...
                <div className="flex h-full flex-col gap-4">
                  ...
                  <div className="grid flex-1 grid-cols-2 gap-4">
                    <div className="flex flex-col justify-between rounded-xl bg-white/5 p-4">
                      <div className="text-slate-500 text-xs">知识点掌握情况</div>
                    ...
                    <div className="flex flex-col justify-between rounded-xl bg-white/5 p-4">
                      ...
                      <div className="font-bold text-3xl text-green-400">+45 分</div>
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((j) => (
                          <div className={`h-4 flex-1 rounded-sm ${j <= 4 ? "bg-green-400/50" : "bg-white/5"}`} key={j} />
                        ))}
                      </div>
                    </div>
                    <div className="col-span-2 rounded-xl bg-white/5 p-4">
                      <div className="mb-4 text-slate-500 text-xs">孩子最近哪些题还容易错</div>
                      <div className="space-y-3">
                        {[
                          ...
                          <div className="flex items-center gap-4" key={item.label}>
                            <div className="w-24 truncate text-[10px] text-slate-400">{item.label}</div>
