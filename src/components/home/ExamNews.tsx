          ...
          {news.map((item) => (
            <div key={item.title}>
              <div className="relative mb-4 aspect-video overflow-hidden rounded-xl">
                <Image alt={item.title} className="object-cover" fill sizes="(min-width: 768px) 33vw, 100vw" src={item.image} />
              </div>
              <div className="mb-2 flex items-center gap-3">
                <span className="font-bold text-primary text-xs uppercase tracking-wider">{item.category}</span>
