import { Clock3Icon, MapPinnedIcon, MessageSquareMoreIcon, PhoneCallIcon, SchoolIcon } from "lucide-react";
import type { Metadata } from "next";
import { PageTopNav } from "~/components/PageTopNav";
import { PhoneButton, PhoneLink } from "~/components/phone-action";
import { CONTACT_CHANNELS, CONTACT_HEADQUARTERS, CONTACT_PAGE_INTRO, CONTACT_TOPICS } from "~/lib/constants/contact";
      ...
      <section className="bg-white">
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="max-w-4xl">
            <div className="inline-flex rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 font-medium text-primary text-sm">{CONTACT_PAGE_INTRO.badge}</div>
            <h1 className="mt-5 font-bold text-3xl text-slate-900 leading-tight md:text-5xl">{CONTACT_PAGE_INTRO.title}</h1>
            <p className="mt-5 max-w-3xl text-base text-slate-600 leading-8 md:text-lg">{CONTACT_PAGE_INTRO.description}</p>
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
            <div className="space-y-6 rounded-3xl border border-slate-200 bg-white p-6 md:p-8">
              ...
              {visibleChannels.map((item) => {
                const Icon = CONTACT_ICONS[item.label as keyof typeof CONTACT_ICONS] ?? MessageSquareMoreIcon;
                ...
                return (
                  <div className="flex items-start gap-4" key={item.label}>
                    ...
                    <div>
                      ...
                      {item.label === "咨询热线" ? (
                        <PhoneLink className="mt-1 inline-block font-bold text-2xl text-slate-900">{item.value}</PhoneLink>
                      ) : item.href ? (
                        <a className="mt-1 inline-block font-semibold text-slate-900 transition-colors hover:text-primary" href={item.href} rel="noopener noreferrer" target="_blank">
                          {item.value}
                        </a>
                      ) : (
                        <div className="mt-1 font-semibold text-slate-900">{item.value}</div>
                      )}
                      {item.description ? <div className="mt-1 text-slate-500 text-sm leading-6">{item.description}</div> : null}
                    </div>
                  </div>
                );
              })}
              ...
              {hasTopics ? (
                <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-8">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-semibold text-slate-900 text-xl">可以咨询这些问题</h2>
                  </div>
              ...
              {CONTACT_HEADQUARTERS.navigationNote ? (
                <div className="rounded-3xl border border-slate-200 bg-white p-6 md:p-8">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="font-semibold text-slate-900 text-xl">到访说明</h2>
                  </div>
                  <p className="mt-4 text-slate-600 leading-8">{CONTACT_HEADQUARTERS.navigationNote}</p>
                  {hasNavigation ? (
                    <div className="mt-6">
                      <a
