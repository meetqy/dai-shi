                  ...
                  <div className="border-slate-200 border-t">
                    {relatedQuestions.map((item) => (
                      <div className="border-slate-200 border-b py-6" key={item.href}>
                        <h3 className="font-bold text-lg text-slate-900 leading-7">{item.question}</h3>
...
function CampusDirectoryTable({ section }: { section: CampusDirectorySection }) {
  return (
    <div className="space-y-4">
      {section.note ? <p className="text-slate-500 text-sm leading-7">{section.note}</p> : null}
      <div className="overflow-x-auto rounded-2xl border border-slate-200">
        <table className="min-w-full border-collapse text-left text-sm">
          <thead className="bg-slate-50/70 text-slate-600">
            <tr>
              <th className="border-slate-200 border-b px-4 py-3 font-medium">城市</th>
              <th className="border-slate-200 border-b px-4 py-3 font-medium">城区</th>
              <th className="border-slate-200 border-b px-4 py-3 font-medium">校区名称</th>
              <th className="border-slate-200 border-b px-4 py-3 font-medium">校区属性</th>
              <th className="border-slate-200 border-b px-4 py-3 font-medium">校区地址</th>
            </tr>
          </thead>
          <tbody className="bg-white">
            {section.rows.map((row) => (
              <tr className="border-slate-100 border-b last:border-b-0" key={`${row.city}-${row.district}-${row.campusName}`}>
                <td className="px-4 py-4 text-slate-700">{row.city}</td>
                <td className="px-4 py-4 text-slate-700">{row.district}</td>
                <td className="px-4 py-4 font-medium text-slate-900">{row.campusName}</td>
                <td className="px-4 py-4 text-slate-700">{row.attribute}</td>
                <td className="px-4 py-4 text-slate-700 leading-7">{row.address}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function CampusDirectoryBlock({ article }: { article: JiaZhangArticle }) {
  ...
  const heroButtonText = hero?.primaryButtonText ?? "立即电话咨询";
  const heroStats = hero?.stats ?? [
    { label: "校区总数", value: `${sections.reduce((sum, section) => sum + section.rows.length, 0)} 个` },
  ...
  return (
    <>
      ...
      <article className="bg-white">
        ...
        <div className="container mx-auto px-4 py-12 md:py-16">
          <div className="grid gap-12 lg:grid-cols-[240px_minmax(0,1fr)]">
            <TableOfContents items={sections.map((section) => ({ id: section.id, title: section.title }))} title={tocTitle} />

            <div className="space-y-20">
              {sections.map((section, sectionIndex) => (
                <section className={cn("scroll-mt-48 py-2", sectionIndex % 2 === 1 && "rounded-2xl bg-slate-50/70 px-6 py-10 md:px-8")} id={section.id} key={section.id}>
                  <div className="relative mb-10 overflow-hidden">
                    ...
                    <div className="relative max-w-3xl">
                      <div className="mb-4 inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-slate-600 text-sm">{section.label ?? "校区分组"}</div>
                      <h2 className="font-bold text-3xl text-slate-900 leading-tight md:text-[2rem]">{section.title}</h2>
                      <p className="mt-4 max-w-2xl text-slate-500 leading-7">{section.description}</p>
                    </div>
                  </div>
...
export default async function JiaZhangArticleDetailPage({ params }: PageProps) {
  ...
  return (
    <div className={`min-h-screen ${article.content.kind === "faq" ? "bg-slate-50" : "bg-white"}`}>
        ...
        {article.content.kind === "faq" ? (
          <div className="py-12">
            <div className="mb-12 text-center">
              <div className="mb-4 inline-flex items-center rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 font-medium text-primary text-sm">戴氏教育 · 家长问答</div>
