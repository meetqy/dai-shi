...
export const metadata: Metadata = {
  title: "荣誉资质 - 戴氏教育高考总部",
  description: "查看戴氏教育高考总部荣誉证书、版权登记、办学资质、质量认证、媒体报道与行业任职等真实展示内容。",
};
...
function HonorCard({ index }: { index: number }) {
  ...
  const gridClassName = isReversed ? "grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,520px)] lg:gap-12" : "grid items-start gap-8 lg:grid-cols-[minmax(0,520px)_minmax(0,1fr)] lg:gap-12";
  const imageWrapperClassName = isReversed ? "lg:order-2 lg:w-full lg:max-w-[520px] lg:justify-self-end" : "lg:w-full";
      ...
      <div className={gridClassName}>
        <div className={imageWrapperClassName}>
          <a className="group block" href={honor.imageSrc} rel="noopener noreferrer" target="_blank">
            ...
            <p className="mt-3 text-slate-500 text-sm transition-colors group-hover:text-primary">点击查看大图</p>
          </a>
        </div>

        <div className={contentWrapperClassName}>
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <span className="inline-flex items-center bg-primary/8 px-3 py-1 font-medium text-primary text-sm">{honor.category}</span>
          ...
          <h2 className="text-balance font-bold text-2xl text-slate-950 leading-tight md:text-3xl">{honor.title}</h2>
          <p className="mt-4 font-medium text-lg text-slate-800 leading-relaxed">{honor.headline}</p>
          ...
          <div className="mt-8 border-slate-200 border-t pt-6">
            ...
            <dl className="mt-4 space-y-3">
              <div className="grid gap-2 md:grid-cols-[132px_minmax(0,1fr)]">
                ...
                <dd className="font-medium text-slate-900 leading-7">{honor.issuer}</dd>
              </div>
              {honor.facts.map((fact) => (
                <div className="grid gap-2 md:grid-cols-[132px_minmax(0,1fr)]" key={`${honor.id}-${fact.label}`}>
      ...
      <main className="pb-16">
        <section className="bg-white">
          <div className="container mx-auto px-4 py-10 md:py-14">
            ...
            <h1 className="mt-3 max-w-4xl text-balance font-bold text-4xl text-slate-950 leading-tight md:text-5xl">戴氏教育高考总部荣誉证书展示</h1>
            <p className="mt-5 max-w-4xl text-lg text-slate-600 leading-8">
              这里集中展示戴氏教育高考总部的办学资质、版权登记、质量认证、行业任职、品牌荣誉与媒体报道。 从长期办学到多维背书，让家长与学生更直观看见品牌沉淀与办学实力。
            </p>
