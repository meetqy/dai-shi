    ...
    <div className="bg-slate-50 pb-12 md:pb-20">
      ...
      <div className="container mx-auto px-4 pt-8 md:pt-12">
        <div className="mb-12 text-center">
          <h1 className="mb-4 font-bold text-3xl text-slate-900 md:text-5xl">价格表</h1>
          <p className="mx-auto max-w-2xl text-lg text-slate-600">为您提供${SITE_FULL_NAME}各校区各类课程的价格信息与优惠政策。</p>
        </div>

        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {priceListItems.map((item) => (
            <SimpleCard description={item.description} href={`/jia-ge-biao/${item.slug}`} key={item.slug} meta={`${item.campus} · ${item.category}`} title={item.title} />
          ))}
